...
for(i in 1:32){
  ...
  #preallocate IDs and timestamp
  ID <- list()
  DateTime <- list()
  #extract incubation time and depth
  for (k in 1:length(files)){
    ID[[k]] <- xlsx_cells(files[k], sheet = 1)
    DateTime[[k]] <- xlsx_cells(files[k], sheet = 1)
    ID[[k]] <- ID[[k]][ID[[k]]$data_type == "character",c("address", "character")] %>%
      filter(address == "A10" | address == "A11") %>% 
      pull(character) %>% substring(6)
    DateTime[[k]] <- DateTime[[k]][DateTime[[k]]$data_type == "character",c("address", "character")] %>%
      filter(address == "A7" | address == "A8") %>% 
      pull(character) %>% substring(7)
  }
  # merge DateTime
  for(x in 1:length(files)){
    DateTime[[x]] <- paste(DateTime[[x]][1],DateTime[[x]][2])
  }
  # ID prep
  ...
  # adding the ID, the incubation time and the DateTime of the measurement to the main list of tables
  for (m in 1:length(files)){
    enz[[m]] <- enz[[m]] %>% mutate(ID = ID[[m]][1], Time = lstTime[[m]], DateTime = dmy_hms(DateTime[[m]]))
  }
  # merging the list of tibbles
  enz <- data.table::rbindlist(enz) %>% as_tibble()
  
  # adding station number
  enz <- enz %>% mutate(Station = 5)
...
enz %>% summarise(unique(depth))
  ...
  # Cleanup
  enz <- enz %>% 
    select(Station, Well, depth, Time,DateTime, enzyme, sizefraction, gain1) 
  # subtract T6 from T0 
  enz %>% 
    arrange(Station,Time) %>% 
    group_by(Well, depth) %>% 
    mutate(lead_time = DateTime - lag(DateTime)) %>% 
    filter(Time == 6)
   
  
  # creating excel files for possible errors and stations 
  file <- paste("Stations_clean/Station_",i,".xlsx")
  errorfile <- paste("Suspiscious_Measurements/Station_",i,"errors.xlsx")
